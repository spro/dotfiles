...
local showHintMod = {'ctrl', 'alt', 'cmd'}
local showHintKey = 'Space'
...
function hintableWindow(window)
    if (window:application():title() == "") then
        print("what is this", window:title())
    elseif (window:title() == "") then
        print("or this", window:application():title())
    end
    return (window:title() ~= "") and (window:application():title() ~= "") and (true or window:isStandard())
end
